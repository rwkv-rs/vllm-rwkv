...
// Dense 3D mix, 2D KK/a grid, warp LNX, and vectorized v-residual kernels
// follow faster3a_2607 at commit
...
constexpr int VRES_VEC2_C = 4096;
constexpr int VRES_VEC2_MIN_ROWS = 64;
constexpr int VRES_VEC2_LARGE_MIN_ROWS = 256;
constexpr int VRES_VEC2_MAX_ROWS = 65535;
constexpr int VRES_VEC2_SMALL_THREADS = 128;
constexpr int VRES_VEC2_LARGE_THREADS = 256;
...
__global__ void tmix_mix6_3d_kernel(
  ...
  if constexpr (UpdateShift) {
    // This specialization is only launched for T==1, so every half2 has one
    // owner that reads its old state before replacing it.
...
__global__ void tmix_kk_a_gate_2d_kernel(
    ...
    const dtype* __restrict__ a0, const dtype* __restrict__ a12,
    const dtype* __restrict__ k_a, dtype* __restrict__ new_k,
    dtype* __restrict__ neg_kk, dtype* __restrict__ kka) {
  const int warp = threadIdx.x >> 5;
  const int lane = threadIdx.x & 31;
  ...
  const int64_t base = bth * HEAD_SIZE;
  const int64_t c =
      static_cast<int64_t>(h) * HEAD_SIZE + static_cast<int64_t>(lane) * 2;
  const int64_t idx = base + static_cast<int64_t>(lane) * 2;
...
__global__ __launch_bounds__(32) void tmix_lnx_rkvres_xg_warp_kernel(
    int H, const dtype* __restrict__ x, const dtype* __restrict__ r,
    const dtype* __restrict__ k, const dtype* __restrict__ v,
    const dtype* __restrict__ r_k, const dtype* __restrict__ weight,
    const dtype* __restrict__ bias, const dtype* __restrict__ g,
    dtype* __restrict__ out, int64_t bth_size) {
...
__global__ __launch_bounds__(Threads) void tmix_vres_gate_vec2_kernel(
    int C, const dtype* __restrict__ v, const dtype* __restrict__ v_first,
    const dtype* __restrict__ v0, const dtype* __restrict__ v12,
    dtype* __restrict__ out, int64_t rows) {
  ...
  store_h2(out + idx, fmaf(vf.x - vv.x, gate0, vv.x),
...
bool is_half2_compatible(const at::Tensor& tensor) {
  ...
  const auto address =
      reinterpret_cast<std::uintptr_t>(tensor.data_ptr<dtype>());
  return address % alignof(__half2) == 0;
}
...
__global__ void cmix_mix_3d_kernel(int T, int C, const dtype* __restrict__ x,
  ...
  if constexpr (UpdateShift) {
    // Only T==1 launches this specialization; no other CTA reads this state.
    *reinterpret_cast<__half2*>(shift_state + static_cast<int64_t>(b) * C + c) =
        cur2;
  }
}
...
__global__ void add_vec_2d_kernel(int C, const dtype* __restrict__ x,
                                  const dtype* __restrict__ vec,
                                  dtype* __restrict__ out) {
  const int c_pair = static_cast<int>(blockIdx.x) * blockDim.x + threadIdx.x;
  const int c_pairs = C >> 1;
  ...
  // The host contract limits complete C-wide rows to the portable grid.y
  // range. Making row a grid coordinate removes the 64-bit modulo per half2.
  const int64_t row = blockIdx.y;
  const int c = c_pair << 1;
  const int64_t idx = row * C + c;
  const float2 xv = __half22float2(load_h2(x + idx));
...
#pragma unroll
  ...
  *reinterpret_cast<int*>(&acc) = 0;
  if constexpr (Split2) {
    // Two independent FP16 accumulation chains shorten the loop-carried
    // dependency. The final add deliberately matches faster3a_2607.
    __half2 acc1;
...
#pragma unroll
  ...
  *reinterpret_cast<int*>(&acc) = 0;
  if constexpr (Accumulators == 1) {
    ...
    static_assert(Accumulators == 2,
                  "production t512 supports one or two accumulators");
    __half2 acc1;
    *reinterpret_cast<int*>(&acc1) = 0;
    for (int i = 0; i < nnz_count; i += 2) {
      const int local_f0 = nnz_ids[i];
...
#pragma unroll
  ...
  // Preserve each warp ballot across the prefix scan so the compaction pass
  // does not reload, retest, and re-ballot the same values.
  unsigned masks[2];
#pragma unroll
  for (int u = 0; u < 2; ++u) {
    const int local_f = tid + u * THREADS;
  ...
  if (tid == 0) {
    int count = 0;
#pragma unroll
    for (int warp = 0; warp < TILE / 32; ++warp) {
      warp_prefix[warp] = count;
      count += warp_counts[warp];
    }
    nnz_count = count;
  }
  __syncthreads();
...
std::vector<at::Tensor> tmix_mix6_3d_cuda(int B, int T, int C, at::Tensor x,
  ...
  // retains its slot-aware path and tuned auto-routing.
  if (T == 1) {
...
std::vector<at::Tensor> tmix_kk_a_gate_2d_cuda(int B, int T, int C, int H,
  ...
  tmix_kk_a_gate_2d_kernel<<<grid, WARPS_PER_BLOCK * 32, 0, stream>>>(
      H, k.data_ptr<dtype>(), k_k.data_ptr<dtype>(), a0.data_ptr<dtype>(),
      a12.data_ptr<dtype>(), k_a.data_ptr<dtype>(), new_k.data_ptr<dtype>(),
      neg_kk.data_ptr<dtype>(), kka.data_ptr<dtype>());
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return {new_k, neg_kk, kka};
}
...
at::Tensor tmix_lnx_rkvres_xg_warp_cuda(int B, int T, int C, int H,
  ...
  tmix_lnx_rkvres_xg_warp_kernel<<<static_cast<int>(bth_size), 32, 0, stream>>>(
      H, x.data_ptr<dtype>(), r.data_ptr<dtype>(), k.data_ptr<dtype>(),
      v.data_ptr<dtype>(), r_k.data_ptr<dtype>(), weight.data_ptr<dtype>(),
      bias.data_ptr<dtype>(), g.data_ptr<dtype>(), out.data_ptr<dtype>(),
      bth_size);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
...
at::Tensor tmix_vres_gate_cuda(int B, int T, int C, at::Tensor v,
  ...
  auto stream = at::cuda::getCurrentCUDAStream();
  // faster3a_2607 keeps 16 CTAs per C=4096 row with vec2/128 below 256 rows,
  // then uses vec2/256 once row-level task supply is sufficient. The row cap
  ...
  if (use_vec2) {
    ...
    const int blocks =
        static_cast<int>(ceil_div(total, VRES_VEC2_LARGE_THREADS));
    tmix_vres_gate_kernel<<<blocks, VRES_VEC2_LARGE_THREADS, 0, stream>>>(
...
void cmix_sparse_down_relu_one_out_cuda(int C, int F, at::Tensor preact,
...
at::Tensor cmix_sparse_down_relu_rows_t512_cuda(int B, int T, int C, int F,
  ...
  zero_vec4_kernel<<<static_cast<int>(ceil_div(out_vec4, 128)), 128, 0,
                     stream>>>(out.data_ptr<dtype>(), out_vec4);
  const bool acc2 = use_cmix_t512_acc2(B, T);
  if (use_cmix_t512_reuse(B, T)) {
    if (acc2) {
...
at::Tensor cmix_mix_3d_cuda(int B, int T, int C, at::Tensor x,
                            at::Tensor shift_state, at::Tensor x_k) {
  auto out = at::empty_like(x);
  constexpr int threads = 256;
  const int c_pairs = C / 2;
