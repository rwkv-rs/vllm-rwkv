...
def resolve_rwkv_tool_parser(
    *,
    ...
    # A raw model filename is not a tokenizer or architecture identity. CLI
    # callers without ModelConfig must select the standard RWKV tokenizer mode.
    if _is_rwkv_tokenizer_mode(tokenizer_mode):
        return RWKV_TOOL_CALL_PARSER
    return tool_parser
