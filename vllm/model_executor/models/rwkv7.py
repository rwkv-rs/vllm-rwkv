...
class RWKV7ForCausalLM(nn.Module):
    ...
    def _preprocess_weights(self, z: dict[str, torch.Tensor]) -> None:
        ...
        # NVFP4 offline-quantized model detection + orig-linear weight preprocessing:
        # Standard NVIDIA NVFP4 format - E2M1 4-bit weights (uint8 packed [N, K/2])
        # with E4M3 block scales (per 16 elements) + FP32 tensor-scale
        # (per-tensor scalar). Dequant: value = e2m1[code] * b_scale_e4m3 * t_scale.
        self._is_nf4 = any(k.endswith(".nvfp4_t_scale") for k in z)
        self.nf4_block_scales: dict[int, torch.Tensor] = {}
        self.nf4_tensor_scales: dict[int, float] = {}
        if self._is_nf4:
            nf4_count = 0
            dev = first_device()
            for key in list(z.keys()):
                bscale_key = key + ".nf4_b_scale"
                tscale_key = key + ".nvfp4_t_scale"
                if bscale_key not in z:
                    continue
                w_raw = z[key].squeeze()
                b_scale = z[bscale_key].squeeze()
                t_scale = float(z[tscale_key].squeeze().item())

                w_raw = self._shard_weight_for_tp(key, w_raw)
                b_scale = self._shard_nf4_bscale_for_tp(key, b_scale)
                w_raw = w_raw.to(device=dev).contiguous()
                b_scale = b_scale.to(
                    device=dev, dtype=torch.float8_e4m3fn
                ).contiguous()
                z[key] = w_raw
                z[bscale_key] = b_scale
                z[tscale_key] = torch.tensor(
                    t_scale, dtype=torch.float32, device=dev
                )
                nf4_count += 1
                logger.info(
                    "RWKV7 nvfp4 weight loaded: %s shape=%s t_scale=%s",
                    key, tuple(w_raw.shape), t_scale,
                )
            logger.info(
                "RWKV7 nvfp4 quantized model detected; %d orig-linear nvfp4 weights",
                nf4_count,
            )
        ...
        for key in list(z.keys()):
            if not self._is_weight_needed_on_rank(key):
                del z[key]
                continue
            # NVFP4: skip uint8 weights and scale tensors (already processed above)
            if self._is_nf4 and (
                getattr(z.get(key), "dtype", None) == torch.uint8
                or key.endswith(".nf4_b_scale")
                or key.endswith(".nvfp4_t_scale")
            ):
                continue
            value = z[key].squeeze()
        ...
        if getattr(self, "_is_nf4", False):
            _E2M1_BOUNDS = torch.tensor(
                [0.25, 0.75, 1.25, 1.75, 2.5, 3.5, 5.0], device=first_device()
            )
            _requant_count = 0
            for _vw_key in list(z.keys()):
                if _vw_key.endswith(".ffn.value.weight") and z[_vw_key].dtype == torch.uint8:
                    ...
                    if _bs_key in z and _ts_key in z:
                        ...
                        _w_fc = torch.ops.rwkv7_nf4_ops.dequant_nf4_to_f16(
                            z[_vw_key], _bs, _ts, True
                        )
                        ...
                        z[_ts_key] = torch.tensor(
                            _ts_new, dtype=torch.float32, device=_w_fc.device
                        )
                        _requant_count += 1
            if _requant_count:
                logger.info(
                    "RWKV7 nvfp4 transpose-requant: %d ffn.value.weight tensors "
                    "re-quantized to [F, C/2] layout for cmix kernel",
                    _requant_count,
                )

        # NVFP4: rebuild block scale + tensor scale lookups (uint8 weight -> scales)
        if getattr(self, "_is_nf4", False):
            for k, w in self.z.items():
                if w.dtype == torch.uint8 and (k + ".nf4_b_scale") in self.z:
                    self.nf4_block_scales[id(w)] = self.z[k + ".nf4_b_scale"]
                    if (k + ".nvfp4_t_scale") in self.z:
                        self.nf4_tensor_scales[id(w)] = float(
                            self.z[k + ".nvfp4_t_scale"].item()
                        )

    def _shard_nf4_bscale_for_tp(self, key: str, b_scale: torch.Tensor) -> torch.Tensor:
        """Shard NVFP4 block scales for tensor parallelism."""
        if getattr(self, "tp_size", 1) == 1:
            return b_scale
        if key == "head.weight":
    ...
    def tmix_varlen(
        ...
        *,
        ...
        if wr.dtype == torch.uint8 and path.rows == 1:
            ...
            r = self.linear(xr, wr)
            k = self.linear(xk, wk_)
            v = self.linear(xv, wv_)
        local_c = r.shape[-1]
        local_h = local_c // N
    ...
    def cmix_from_mixed(
        ...
        # NVFP4 value.weight: dispatch to NVFP4 cmix_sparse kernels
        if vw.dtype == torch.uint8:
            b_scale = self.nf4_block_scales.get(id(vw))
            if b_scale is not None:
                t_scale = self.nf4_tensor_scales.get(id(vw), 1.0)
                ...
                if path.cmix_mode == CMIX_B1T1_NOFC:
                    return self._tp_all_reduce(
                        nf4_ops.cmix_sparse_down_relu_one_nf4(
                            C, z[p + "value.weight"].size(0),
                            hid.view(-1).contiguous(), vw, b_scale, t_scale,
                        )
                    )
                    ...
                    if path.rows >= CMIX_NOFC_T512_MIN_ROWS and C % 512 == 0 and F % 512 == 0:
                        return self._tp_all_reduce(
                            nf4_ops.cmix_sparse_down_relu_rows_t512_nf4(
                                B, T, C, F, hid.contiguous(), vw, b_scale, t_scale,
                            )
                        )
                    return self._tp_all_reduce(
                ...
                # dense: dequant and fall through
                vw = self._dequant_nf4_value(vw)
        if path.cmix_mode == CMIX_B1T1_NOFC:
            return self._tp_all_reduce(
    ...
    def linear(self, x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        if weight.dtype == torch.uint8:
            return self.linear_orig_layout(x, weight, PathConfig(1, False, CMIX_DENSE), "att_c2c")
    ...
    def linear_orig_layout(
        ...
        """Linear with NVFP4 dispatch for orig-linear weights."""
        if weight.dtype != torch.uint8:
            # FP16 weight: use standard linear
            if x.numel() == x.size(-1) and weight.size(1) % 64 == 0:
                return torch.ops.rwkv7_v3a_ops.linear_f16_m1_splitk(x.contiguous(), weight)
            return torch.ops.rwkv7_v3a_ops.linear_f16(
                x.contiguous(), weight, self.allow_fp16_accumulation
        ...
        # NVFP4 orig-linear weight: rows 1/2 use the optimized NVFP4 blk16
        # GEMV kernel; rows 3-12 use the NF4 GEMM kernel; rows > 12 dequant.
        xc = x.contiguous()
        b_scale = self.nf4_block_scales.get(id(weight))
        if b_scale is None:
            # No block scale found, dequant and fall through
            return torch.ops.rwkv7_v3a_ops.linear_f16(
                xc, weight, self.allow_fp16_accumulation
            )
        t_scale = self.nf4_tensor_scales.get(id(weight), 1.0)
        if path.rows == 1:
        ...
        w_fp16 = torch.ops.rwkv7_nf4_ops.dequant_nf4_to_f16(
            weight, b_scale, t_scale, False
        )
        return torch.ops.rwkv7_v3a_ops.linear_f16(
            xc, w_fp16, self.allow_fp16_accumulation
        )
    ...
    def _dequant_nf4_value(self, vw: torch.Tensor) -> torch.Tensor:
        """Dequant NVFP4 value.weight to fp16 for sparse cmix paths."""
        if vw.dtype == torch.uint8:
            b_scale = self.nf4_block_scales.get(id(vw))
            if b_scale is not None:
                t_scale = self.nf4_tensor_scales.get(id(vw), 1.0)
                return torch.ops.rwkv7_nf4_ops.dequant_nf4_to_f16(
                    vw, b_scale, t_scale, False
                )
        return vw

    def linear_head(self, x: torch.Tensor) -> torch.Tensor:
        w = self.z["head.weight"]
        if w.dtype == torch.uint8:
            # head.weight must stay FP16 per project constraint
            w = self._dequant_nf4_value(w) if hasattr(self, "_is_nf4") and self._is_nf4 else w
