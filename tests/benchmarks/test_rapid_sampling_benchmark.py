...
def test_run_config_records_legacy_wrapper_penalty_diagnostics(monkeypatch) -> None:
    config = rapid_bench.BenchmarkConfig(
        batch_size=3,
        vocab_size=8,
        top_p=0.9,
    ...
    def fake_rapid_sample(
        ...
        **_kwargs,
        ...
        assert penalty_indices is None
        assert penalties.shape == (config.batch_size, config.vocab_size)
        return real_zeros((config.batch_size,), dtype=torch.int32)
    ...
    assert result["rapid_penalty_index_rows"] == config.batch_size * 2
    assert result["rapid_penalty_index_first_values"] == [0, 2, 4]
    assert result["rapid_penalty_indexed_legacy_ms"] == 0.75
    assert result["rapid_penalty_indexed_legacy_stats"] == {
        "indexed_calls": 1,
        "indexed_rows": 3,
        "indexed_vocab_elements": 24,
        "wrapper_gather_scatter_calls": 1,
        "wrapper_gather_scatter_elements": 48,
    }
